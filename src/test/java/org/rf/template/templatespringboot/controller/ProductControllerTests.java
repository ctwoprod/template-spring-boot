package org.rf.template.templatespringboot.controller;
...
public class ProductControllerTests {
	...
	public void testFindAll() {
		get(BASE_URL + "/").then().body("totalElements", equalTo(1)).body("content.id", hasItems("abc123"));
	}
	...
	public void testSave() throws Exception {
		...
		px.setCode("PT-001");
		given().body(px).contentType(ContentType.JSON).when().post(BASE_URL + "/").then().statusCode(400);
		...
		given().body(px1).contentType(ContentType.JSON).when().post(BASE_URL + "/").then().statusCode(400);
	...
	public void testFindById() {
		...
		get(BASE_URL + "/990").then().statusCode(404);
	}
	...
	public void testUpdate() {
		...
		given().body(p).contentType(ContentType.JSON).when().put(BASE_URL + "/abc123").then().statusCode(200);

		get(BASE_URL + "/abc123").then().statusCode(200).body("id", equalTo("abc123")).body("code", equalTo("PX-009"))
				.body("name", equalTo("Product 909"));

		// test id salah
		given().body(p).contentType(ContentType.JSON).when().put(BASE_URL + "/xyz456").then().statusCode(404);
	}
	...
	public void testDelete() {
		...
		// test id salah
		delete(BASE_URL + "/xyz123").then().statusCode(404);
	}
}